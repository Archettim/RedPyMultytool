...
class Sidebar(Container):
    def compose(self) -> ComposeResult:
        ...
        yield Button("Smurf (not Implemented yet)",id="smu", variant="success")
        yield DarkSwitch()
    ...
    def on_button_pressed(self,event:Button.Pressed) -> None:
        ...
        if button_id=="dns":
            try:
                self.app.query_one(ARPpoison).visible=False
                self.app.query_one(ReverseSH).visible=False
                self.app.query_one(DNSPoison).visible=False
                self.app.query_one(Ramsomware).visible=False
            except:
                pass
            self.app.query_one(DNSPoison).visible=True
            self.add_class("dnsp")
            self.remove_class("rams")
            self.remove_class("shell")
            self.remove_class("arpp")
            self.remove_class("smrf")
        elif button_id=="arp":
            try:
                self.app.query_one(DNSPoison).visible=False
                self.app.query_one(ReverseSH).visible=False
                ...
                pass
            self.app.query_one(ARPpoison).visible=True
            self.add_class("arpp")
            self.remove_class("shell")
            self.remove_class("rams")
            ...
            try:
                ...
                pass
            self.app.query_one(ReverseSH).visible=True
            ...
            try:
                ...
                pass
            self.app.query_one(Ramsomware).visible=True
            self.add_class("rams")
            self.remove_class("arpp")
            self.remove_class("dnsp")
            self.remove_class("shell")
    ...
    def on_button_pressed(self,event:Button.Pressed) -> None:
        ...
        if button_id=="ARPsubm":
            ...
            try:
                ...
                self.tl2.write("ERROR: can't start the poisoning")
        elif(button_id=="ARPstop"):
            try:
                self.arpois.ARPstop()
                self.tl2.write("ARP poisoning stoped...")
            except:
                self.tl2.write("ERROR: process is not running")
...
class Ramsomware(Static):
    ...
    def on_button_pressed(self,event:Button.Pressed) -> None:
        ...
        if button_id=="genKEY":
            ...
            self.ciph.crypt(self.inp2.value,self.tl1)
        elif(button_id=="decr"):
            self.ciph.decrypt(self.inp2.value,self.tl1)
